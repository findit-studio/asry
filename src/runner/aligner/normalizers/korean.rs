...
impl TextNormalizer for KoreanNormalizer {
  ...
  fn normalize<'a>(&self, text: &'a str) -> Result<NormalizedText<'a>, NormalizationError> {
    ...
    // hit the wav2vec2-large-xlsr-53-korean vocab the same way
    ...
    // FinDIT-Studio ONNX ships is precomposed-syllable-level,
    // matching the `jonatasgrosman/wav2vec2-large-xlsr-53-korean`
    // upstream tokenizer.
    for c in text.chars() {
