...
//! produces it. Whispery re-exports here so existing call sites
//! (`whispery::Lang`) continue to compile unchanged. The serde
//! impls — lowercase ISO-639-1 wire format, case-insensitive
//! deserialise, validation against `[a-zA-Z]{1,8}` — are gated
//! behind whispery's `serde` feature, which chains to
//! `whisper-cpp/serde`.
//!
