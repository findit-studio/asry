...
#
# Override the source via WHISPERY_FIXTURES_REPO_URL /
# WHISPERY_FIXTURES_REPO_REF (defaults below). Set
# WHISPERY_FIXTURES_KEEP_CLONE=1 to leave the shallow clone
# under `target/audio-fixtures/` for inspection; default
# behaviour cleans it up after the copy.

set -euo pipefail

REPO_URL="${WHISPERY_FIXTURES_REPO_URL:-https://github.com/Findit-AI/audio-fixtures.git}"
REPO_REF="${WHISPERY_FIXTURES_REPO_REF:-main}"
...
if [ -d "$DEST" ] && [ "${WHISPERY_FIXTURES_FORCE:-0}" != "1" ]; then
  if [ -f "$DEST/01_dialogue/clip_16k.wav" ] && [ -f "$DEST/01_dialogue/reference.rttm" ]; then
    echo "fetch_fixtures: $DEST already populated; set WHISPERY_FIXTURES_FORCE=1 to refetch."
    exit 0
  fi
fi
...
if [ "${WHISPERY_FIXTURES_KEEP_CLONE:-0}" != "1" ]; then
  rm -rf "$CLONE_DIR"
fi
